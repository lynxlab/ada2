 ...
 */
public function add_openmeetings_room($name="service", $sess_id_course_instance, $sess_id_user, $comment="Inserimento automatico via ada",$num_user=2) {
	$dh            =   $GLOBALS['dh'];
    $error         =   $GLOBALS['error'];
    $debug         =   $GLOBALS['debug'];
    $root_dir      =   $GLOBALS['root_dir'];
...
public function room_access($username,$nome,$cognome,$user_email) {
 ...
 */
	 ...
	 * LINK A STANZA
	 */
	$this->link_to_room = "http://".$host.$port."/".$dir."/main.lzx.lzr=swf8.swf?roomid=".$this->id_room."&sid=".$this->session_id."&language=".ROOM_DEFAULT_LANGUAGE;
