...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($form->isValid()) {
		...
		if (isset($_POST['forceSaveExtra']) && $userObj->hasExtra()) $userObj->setExtras($_POST);
		
		MultiPort::setUser($userObj, array(), true, ADAUser::getExtraTableName());
		$_SESSION['sess_userObj'] = $userObj;
		// if registration form is saved ok, force a page
		// reload to reflect the changes immediately 
