...
//    return $user_id;
    ...
    public function get_log_videoroom($id_instance, $id_room = null, $id_user = null) {
        ...
        $retArrIdx = 0;
        $roomCache = [];
        // 00. get log events generated by a tutor
        $sql = 'SELECT * FROM `log_videochat` WHERE `is_tutor`=? AND `id_istanza_corso`=?';
        $values = [true, $id_instance];
        if (!is_null($id_room)) {
            ...
            foreach ($showChatRooms as $showChatRoom) {
                ...
                if (!AMA_DB::isError($roomCache[$showChatRoom['id_room']])) {
                    ...
                    $sql = 'SELECT `log_videochat`.*,`utente`.`nome` AS `nome`, `utente`.`cognome` AS `cognome` '.
                           ...
                           'WHERE `is_tutor`=? AND `id_istanza_corso`=? AND `entrata`>=? AND `uscita`<=?';
                    $values = [
                        false, $id_instance, $showChatRoom['entrata'], $showChatRoom['uscita']
                    ];
                    if (!is_null($id_user)) {
