 ...
 */
if ($_SERVER['HTTPS']) {
  header('Location:'.$http_root_dir .'/reserved/');
  exit();
}
...
if ($newsmsg == ''){
   $newsfile = $newsfile; //  txt files in ada browsing directory
...
if ($hlpmsg == ''){
   $helpfile = $helpfile;  //  txt files in ada browsing directory
...
if ($infomsg == ''){
   $infofile = $infofile;  //  txt files in ada browsing directory
...
if(isset($p_login)) {
  ...
  if($username !== FALSE && $password !== FALSE) {
    ...
    if($userObj instanceof ADALoggableUser){
      ...
      $_SESSION['ada_access_from'] = ADA_RESERVED_ACCESS;
 ...
 * Show login page
 */
$form_action = HTTP_ROOT_DIR .'/reserved/index.php';
